...
1. [BBT GitHub Releases](https://github.com/retorquere/zotero-better-bibtex/releases)에서 최신 `.xpi` 파일 다운로드.
    ...
    ![GIT releases 페이지](../.source/extention/BBT/bbt_git_download.png)

2. Zotero 실행 후 `도구(Tools)` -> `플러그인 (Add-ons)` 메뉴 이동.

    ![플러그인 버튼 위치](../.source/extention/BBT/plugin_button.png)

3. 우측 상단 톱니바퀴 아이콘 클릭 -> `Install Add-on From File...` 선택.

    ![플러그인 설치 파일 선택](../.source/extention/BBT/select_extention_install_file.png)
...
- 고유 인용 키(Cite Key) 생성: 저자명, 연도, 제목 등을 조합한 충돌 없는 인용 키 자동 생성.
    ...
    `Better BibTeX 탭`에 위치한 `Citation keys`에서 설정 변경

    추천 패턴: `auth.lower + year + veryshorttitle.lower` (저자소문자 + 연도 + 핵심제목소문자)

    ![고유 인증키 패턴 설정](../.source/extention/BBT/bbt_setting.png)
